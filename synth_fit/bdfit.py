...
class BDSampler(object):
	...
	def mcmc_go(self, nwalk_mult=20, nstep_mult=50, outfile=None):
		...
		#logging.info("avg accept {}".format(np.average(sampler.acceptance_fraction)))
		##logging.info("avg autocorrelation length {}".format(np.average(sampler.acor)))
		...
		self.cropchain = sampler.chain[:,burn_in:,:].reshape((-1,self.ndim))
		...
		if self.snap:
			...
			#logging.debug("Snapped cropchains {} to {}".format(chain_shape,np.shape(self.cropchain)))
